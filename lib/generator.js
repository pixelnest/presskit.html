...
const assetsToCopy = [path.join(__dirname, '../assets/css/master.css'), path.join(__dirname, '../assets/css/normalize.css')]
...
function exploreSync (dirPath, callback) {
  fs.readdirSync(dirPath).forEach(function (name) {
    ...
    if (stat.isFile()) {
      ...
      if (ignoredFolders.includes(p)) {
        console.warn('Ignoring folder ' + filePath)
      } else {
        // Resursivity
