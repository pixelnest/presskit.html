...
function build (basePath, dataFile, presskit, outputJSONfiles) {
  ...
  if (outputJSONfiles) {
    var json = {}
    if (presskit.type === 'company') json.company = presskit
    else if (presskit.type === 'product') json.product = presskit
    else if (presskit.type === 'game') json.game = presskit
    else json = presskit

    fs.writeFileSync(path.join(buildDir, 'data.json'), JSON.stringify(json, null, 2))
  }
}
