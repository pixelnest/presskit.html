...
function build (dataFilePath, presskit, {
  ...
  //
  // TODO: make it optional with a build flag `--no-thumbnails`.
  createThumbnails(assetsTarget, images.screenshots)
...
// Even for gif.
function createThumbnails (target, images) {
  // We don't check if the format is valid, since the check is already done
  // when constructing the images object.
