...
function generate (entryPoint) {
  ...
  if (config.commands.build.watch) {
    ...
    showHeader(`Watching and serving files from http://localhost:${port}/`)
...
function findData (entryPoint) {
  ...
  files.filter(isDataFile).forEach(file => {
    try {
      ...
      // Legacy support
      // If this is a "game" file at top level, it's probably a company
      // Display a warning
      if (path.dirname(file) === entryPoint && presskit.type !== 'company') {
        console.warn('The top level data.xml should start with a <company> and not a <game> tag! ')
        console.warn('If this is a legacy presskit() file, please check the Migration guide (https://github.com/pixelnest/presskit.html#migration-guide).')
