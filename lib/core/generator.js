...
function generateHTML (dataPath, presskits) {
  showHeader('Generating HTML')

  const outputProducts = []

  for (const product of presskits.products) {
    const outputPath = build(dataPath, product.path, product.presskit, [])
    outputProducts.push({path: outputPath, title: product.presskit.title})
  }
