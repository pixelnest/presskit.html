...
function createTemplate (assetsFolder, type) {
  const templatePath = getTemplatePath(assetsFolder, type)

  if (!templatePath) {
    throw new Error('Missing template! Make sure your presskit has a "type" field (product/company)')
  }

  registerPartials(assetsFolder)
...
function registerPartials (assets) {
  const partialsFolder = path.join(assets, '_includes')
